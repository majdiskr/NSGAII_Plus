%NSGA + PSO+ Social Mimicry (SM):
function [delayNSGA2_PSO, energyNSGA2_PSO] = NSGA2_PSO_SM(numVehicles, numTasks, initial_pop_size, some_factor, maxIterations, ...
    ...
    %% Step 2: NSGA-II Optimization Loop with SM Integration
    for iter = 1:maxIterations
        ...
        % Apply Social Mimicry (SM) for Exploration and Diversity
        population = applySocialMimicry(population, numVehicles, numTasks, computingCapacity, taskWorkload);
        ...
        % Display Progress
        fprintf('Iteration %d: Best Fitness (Delay + Energy) NSGA2_PSO_SM: %.2f\n', iter, bestFitness);

        % Check for Convergence
        if abs(prevBestFitness - bestFitness) < convergenceThreshold
            fprintf('Convergence reached at iteration NSGA2_PSO_SM %d\n', iter);
            break;
        end
    ...
    fprintf('Best solution found with Delay_NSGA2_PSO_SM: %.2f and Energy: %.2f\n', alpha.fitness(1), alpha.fitness(2));
    fprintf('Average Energy Consumption_NSGA2_PSO_SM: %.2f\n', averageEnergy);
    fprintf('Average Delay_NSGA2_PSO_SM: %.2f\n', averageDelay);
    fprintf('Standard Deviation of Energy NSGA2_PSO_SM: %.2f\n', stdDevEnergy);
    fprintf('Standard Deviation of Delay NSGA2_PSO_SM: %.2f\n', stdDevDelay);
    fprintf('Total Execution Time NSGA2_PSO_SM: %.2f seconds\n', executionTime);
...
%% Step 4: Apply Social Mimicry (SM) for Diversity
function population = applySocialMimicry(population, numVehicles, numTasks, computingCapacity, taskWorkload)
    % Apply Social Mimicry (SM) to improve diversity and convergence
    mimicRate = 0.2; % Mimicry probability
    eliteThreshold = ceil(0.2 * length(population)); % Top 20% elite individuals

    % Identify top-performing individuals
    elitePopulation = population(1:eliteThreshold); % Top 20% based on sorting

    for i = eliteThreshold + 1:length(population) % Non-elite individuals
        if rand < mimicRate
            % Randomly select an elite individual to mimic
            eliteIdx = randi(eliteThreshold);
            eliteIndividual = elitePopulation(eliteIdx);

            % Mimic the elite individual's position with small random variation
            mimicNoise = randn(1, numTasks) * 0.1; % Add noise for diversity
            population(i).position = eliteIndividual.position + mimicNoise;
...
%% Other Helper Functions (Unchanged from Your Code)
% 1. initializePopulationWithPSO
% 2. psoUpdatePopulation
% 3. evaluateIndividualFitness
% 4. nonDominatedSorting
% 5. tournamentSelection
% 6. crossoverAndMutation
% 7. selectNextGeneration
% 8. findGlobalBestPosition
% 9. dominates
